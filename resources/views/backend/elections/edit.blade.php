...
@section('content')
  <x-backend.card>
      ...
      <x-slot name="body">
        {{ html()->modelForm($election, 'PATCH', route("admin.elections.update", $election))->class('form')->attributes(["enctype" => "multipart/form-data"])->open() }}
        @include ("backend.elections.form")
        {{ html()->button($text = "<i class='fas fa-plus-circle'></i> Ubah Pemilihan", $type = 'submit')->class('btn btn-sm btn-primary float-right') }}
        {{ html()->form()->close() }}
      </x-slot>
